...
$db = new Database();
$db->connect();
$db->insert('ios_ninegame_performance_launch_records',array('testid'=>$testid,'triggertime'=>$triggertime,'duringtime'=>$duringtime,'version'=>$version,'buildid'=>$buildid,'uuid'=>$uuid,'performancedata'=>$performancedata));  // Table name, column names and respective values
$res = $db->getResult();  
print_r($res);
